...
class graphAgent():
    def __init__(self, user_id, profile_url, config):
        ...
        if self.config['model_type'] == 'openai':
            self.llm = OpenAI(user_id=user_id,temperature=0.0, model=self.config['model_name'], api_key=OPENAI_API_KEY)
        ...
        self.linkedin_id = self.profile_url.strip('/').split('/')[-1]


        # Check if the profile already exist in MongoDB
        check_data=mongo_handler.retrieve_relevant_data("general info", self.linkedin_id, "user_experience_rag")

        print("Check data", check_data)
        print(type(check_data))
        print(check_data)
        if check_data=="":
            print("Profile not found in MongoDB. Saving profile to Pinecone")
            save_profile_to_pinecone(dynamic_profile_linkedin, self.linkedin_id)
    ...
    def stream_graph_updates(self, user_input: str):
        ...
        try:
            # Generate an embedding for the query
            profile_experience = mongo_handler.retrieve_relevant_data(user_input, self.linkedin_id, "user_experience_rag")       
            all_content_rag = ' '.join(item['content'] for item in profile_experience)
            print("all_content_rag", all_content_rag)
